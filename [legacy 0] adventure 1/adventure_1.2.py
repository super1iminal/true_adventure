...
#all_locations = middle.extend(north).extend(east).extend(west).extend(south)
player_location = [middle, north, east, west, south][random.randint(0, 4)]
...
def location_change(location_command): #changes your location based on location_commmand input. 
    ...
    if location_command in player_location:
        ...
        if location_command in middle:
            player_location=middle
        if location_command in north:
            player_location=north
        if location_command in east:
            player_location=east
        if location_command in west:
            player_location=west
        if location_command in south:
...
def one_word_commands(command_list):
    ...
    if first_command in middle or north or east or west or south:
